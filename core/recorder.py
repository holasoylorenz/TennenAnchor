"""
Session Video & Animated GIF Recorder for Desktop Control Harness.
...
logger = logging.getLogger("desktop_harness.recorder")
...
class WindowScopedRecorder:
    ...
    def stop(self, filename_prefix: str = "session") -> Optional[Path]:
        ...
        t0 = time.perf_counter()
        logger.info("WindowScopedRecorder: Processing %d captured frames...", len(self._raw_frames))

        processed: List[Image.Image] = []

        for raw_img, status_text in self._raw_frames:
            w, h = raw_img.size
            if w <= 0 or h <= 0:
                continue
            ...
            # Telemetry text
            hud_text = f"Harness 2.0 | {status_text}"
